...
app.post('/api/:shopId/motorbikes', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
...
app.post('/api/:shopId/motorbikes/:bikeId/status', async (req, res) => {
  ...
  try {
    getShop(req.params.shopId);
    const { status, renterName, renterPhone, expectedReturn, price, paymentStatus, pickupLocation } = req.body || {};
...
app.post('/api/:shopId/staff/:staffId/photo', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
  if (!isBossRole(auth)) return res.status(403).json({ error: 'Boss access only' });
  try {
...
app.get('/api/:shopId/motorbikes/:bikeId/deposit', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
  try {
    getShop(req.params.shopId);
...
app.post('/api/:shopId/deposits', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
  try {
    getShop(req.params.shopId);
...
app.post('/api/:shopId/deposits/:depositId/refund', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
  try {
    getShop(req.params.shopId);
...
app.get('/api/:shopId/deposits', async (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.status(401).json({ error: 'Unauthorized' });
  if (!isBossRole(auth)) return res.status(403).json({ error: 'Boss access only' });
  try {
...
function dashboardShell(token, title, bodyAttrs, bodyHTML, isBoss) {
  ...
  return '<!DOCTYPE html><html class="light" lang="en"><head>' +
    ...
    '<header class="flex justify-between items-center w-full px-margin-mobile h-16 z-50 bg-surface border-b border-outline-variant md:hidden sticky top-0"><h1 class="font-headline-lg text-headline-lg font-bold text-primary tracking-tight">TOH Rental</h1><button onclick="document.getElementById(\'sidebar\').classList.remove(\'-translate-x-full\');document.getElementById(\'sidebarBackdrop\').classList.remove(\'hidden\');" class="p-2 -mr-2 text-on-surface"><span class="material-symbols-outlined text-3xl">menu</span></button></header>' +
    '<div id="sidebarBackdrop" onclick="document.getElementById(\'sidebar\').classList.add(\'-translate-x-full\');this.classList.add(\'hidden\');" class="hidden md:hidden fixed inset-0 bg-black/40 z-30"></div>' +
    '<aside id="sidebar" class="flex flex-col h-full py-lg gap-xs bg-surface border-r border-outline-variant fixed left-0 top-0 w-[280px] z-40 overflow-y-auto no-scrollbar transform -translate-x-full md:translate-x-0 transition-transform duration-200"><div class="px-4 mb-6 flex items-center justify-between"><h1 class="font-headline-md text-headline-md text-primary">TOH Rental</h1><button onclick="document.getElementById(\'sidebar\').classList.add(\'-translate-x-full\');document.getElementById(\'sidebarBackdrop\').classList.add(\'hidden\');" class="md:hidden text-on-surface-variant p-1"><span class="material-symbols-outlined">close</span></button></div><nav class="flex flex-col gap-2">' +
    ...
    '<a class="flex items-center gap-4' + active('Calendar') + ' px-4 py-3 mx-2" href="/calendar?token=' + t + '"><span class="material-symbols-outlined">calendar_month</span><span class="font-label-caps text-label-caps">Calendar</span></a>' +
...
app.get('/motorbikes', (req, res) => {
  ...
  if (!auth.ok) return res.redirect('/login');
  const token = req.query.token || '';
  const addBikeBtnHTML = isBossRole(auth) ? '<button onclick="openAddBikeModal()" class="text-xs px-3 py-2 rounded-lg font-status-badge bg-primary text-on-primary hover:opacity-90">+ Add Bike</button>' : '';
  res.send(dashboardShell(token, 'Motorbikes Inventory', '', '<main class="flex-1 md:ml-[280px] pb-24 md:pb-8"><header class="flex flex-col md:flex-row justify-between md:items-center w-full px-margin-mobile md:px-margin-desktop py-3 md:h-16 z-30 bg-surface/80 backdrop-blur-md border-b border-outline-variant md:sticky md:top-0 gap-2"><h2 class="font-headline-md text-headline-md text-on-surface font-semibold">Motorbikes</h2><div class="flex items-center gap-3 flex-wrap"><select id="statusFilter" class="text-sm border border-outline-variant rounded-lg px-3 py-1.5 bg-surface" onchange="render()"><option value="all">All Statuses</option><option value="Available">Available</option><option value="Rented">Rented</option><option value="Maintenance">Maintenance</option><option value="Reserved">Reserved</option></select>' + addBikeBtnHTML + '</div></header><div class="p-margin-mobile md:p-margin-desktop max-w-7xl mx-auto space-y-4"><div class="flex justify-between items-center"><div id="bikeCount" class="text-sm text-on-surface-variant"></div></div><div id="grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div></div></main>' +
  '<div id="addBikeModal" class="fixed inset-0 bg-black/40 items-center justify-center z-50" style="display:none"><div class="bg-surface-container-lowest rounded-xl p-6 w-[300px]"><h3 class="font-headline-md text-headline-md mb-4">Add Bike</h3><input id="newBikePlate" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Plate / code (e.g. Honda Click 125 4001)"><input id="newBikeModel" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Model (e.g. Honda Click 125)"><input id="newBikeColor" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Color (optional)"><input id="newBikeLocation" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Home location (optional)"><div id="addBikeMsg" class="text-xs text-error mb-2"></div><div class="flex gap-2 justify-end"><button class="text-xs px-3 py-2 rounded-lg bg-surface-container-high" onclick="closeAddBikeModal()">Cancel</button><button class="text-xs px-3 py-2 rounded-lg bg-primary text-on-primary" onclick="confirmAddBike()">Add</button></div></div></div>' +
  '<div id="rentModal" class="fixed inset-0 bg-black/40 items-center justify-center z-50" style="display:none"><div class="bg-surface-container-lowest rounded-xl p-6 w-[300px]"><h3 class="font-headline-md text-headline-md mb-4">Mark Rented</h3><input id="rentName" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Customer name"><input id="rentPhone" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Phone (optional)"><input id="rentLocation" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Pickup location"><label class="text-xs text-on-surface-variant">Expected return</label><input id="rentReturn" type="date" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface"><input id="rentPrice" type="number" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Price (THB)"><label class="flex items-center gap-2 text-xs mb-2"><input type="checkbox" id="rentNPY" onchange="document.getElementById(\'rentPrice\').disabled=this.checked;"> Not paid yet (NPY)</label><div class="border-t border-outline-variant my-2 pt-2"><label class="text-xs text-on-surface-variant">Deposit held</label><select id="depositType" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" onchange="document.getElementById(\'depositAmount\').style.display=this.value===\'cash\'?\'block\':\'none\';"><option value="passport">Passport</option><option value="cash">Cash</option></select><input id="depositAmount" type="number" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Deposit amount (THB)" style="display:none"></div><div id="rentMsg" class="text-xs text-error mb-2"></div><div class="flex gap-2 justify-end"><button class="text-xs px-3 py-2 rounded-lg bg-surface-container-high" onclick="closeRentModal()">Cancel</button><button class="text-xs px-3 py-2 rounded-lg bg-primary text-on-primary" onclick="confirmRent()">Confirm</button></div></div></div>' +
  '<div id="returnModal" class="fixed inset-0 bg-black/40 items-center justify-center z-50" style="display:none"><div class="bg-surface-container-lowest rounded-xl p-6 w-[300px]"><h3 class="font-headline-md text-headline-md mb-4">Mark Returned</h3><input id="returnPrice" type="number" class="w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface" placeholder="Final price (THB, optional)"><div id="depositSection" class="border-t border-outline-variant my-2 pt-2"></div><div id="returnMsg" class="text-xs text-error mb-2"></div><div class="flex gap-2 justify-end"><button class="text-xs px-3 py-2 rounded-lg bg-surface-container-high" onclick="closeReturnModal()">Cancel</button><button class="text-xs px-3 py-2 rounded-lg bg-primary text-on-primary" onclick="confirmReturn()">Confirm</button></div></div></div>' +
  ...
  'var rentTarget=null;function openRentModal(id){rentTarget=id;document.getElementById("rentName").value="";document.getElementById("rentPhone").value="";document.getElementById("rentLocation").value="";document.getElementById("rentReturn").value="";document.getElementById("rentPrice").value="";document.getElementById("rentPrice").disabled=false;document.getElementById("rentNPY").checked=false;document.getElementById("depositType").value="passport";document.getElementById("depositAmount").value="";document.getElementById("depositAmount").style.display="none";document.getElementById("rentMsg").textContent="";document.getElementById("rentModal").style.display="flex";}' +
  'function closeRentModal(){document.getElementById("rentModal").style.display="none";}' +
  'function openAddBikeModal(){document.getElementById("newBikePlate").value="";document.getElementById("newBikeModel").value="";document.getElementById("newBikeColor").value="";document.getElementById("newBikeLocation").value="";document.getElementById("addBikeMsg").textContent="";document.getElementById("addBikeModal").style.display="flex";}' +
  'function closeAddBikeModal(){document.getElementById("addBikeModal").style.display="none";}' +
  'async function confirmAddBike(){var plate=document.getElementById("newBikePlate").value.trim();var model=document.getElementById("newBikeModel").value.trim();if(!plate||!model){document.getElementById("addBikeMsg").textContent="Plate and model are required";return;}var body={plate:plate,model:model,color:document.getElementById("newBikeColor").value.trim(),location:document.getElementById("newBikeLocation").value.trim()};var res=await fetch(qs("motorbikes"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var data=await res.json();if(data.error){document.getElementById("addBikeMsg").textContent=data.error;return;}closeAddBikeModal();load();}' +
  'async function confirmRent(){var name=document.getElementById("rentName").value.trim();if(!name){document.getElementById("rentMsg").textContent="Enter customer name";return;}var isNPY=document.getElementById("rentNPY").checked;var priceVal=document.getElementById("rentPrice").value;var body={status:"Rented",renterName:name,renterPhone:document.getElementById("rentPhone").value.trim(),pickupLocation:document.getElementById("rentLocation").value.trim(),expectedReturn:document.getElementById("rentReturn").value,price:priceVal,paymentStatus:isNPY?"npy":(priceVal?"paid":"unpaid")};var res=await fetch(qs("motorbikes/"+rentTarget+"/status"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var data=await res.json();if(data.error||data.ok===false){document.getElementById("rentMsg").textContent=data.error||data.message||"Failed";return;}var depType=document.getElementById("depositType").value;var depAmount=document.getElementById("depositAmount").value;await fetch(qs("deposits"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({plate:rentTarget,renterName:name,depositType:depType,amount:depAmount})});closeRentModal();load();}' +
  'var returnTarget=null;var currentDeposit=null;async function openReturnModal(id){returnTarget=id;currentDeposit=null;document.getElementById("returnPrice").value="";document.getElementById("returnMsg").textContent="";document.getElementById("depositSection").innerHTML="Loading deposit info...";document.getElementById("returnModal").style.display="flex";try{var res=await fetch(qs("motorbikes/"+id+"/deposit"));var data=await res.json();currentDeposit=data.deposit;if(!currentDeposit){document.getElementById("depositSection").innerHTML="<div class=\\"text-xs text-on-surface-variant\\">No deposit on file for this rental</div>";return;}if(currentDeposit.deposit_type==="passport"){document.getElementById("depositSection").innerHTML="<div class=\\"text-xs mb-2\\">Deposit held: <strong>Passport</strong></div><label class=\\"flex items-center gap-2 text-xs mb-2\\"><input type=\\"checkbox\\" id=\\"depositReturned\\" checked> Passport returned to customer</label>";}else{document.getElementById("depositSection").innerHTML="<div class=\\"text-xs mb-2\\">Deposit held: <strong>"+currentDeposit.amount_collected+" THB</strong></div><label class=\\"text-xs text-on-surface-variant\\">Refund amount</label><input id=\\"refundAmount\\" type=\\"number\\" value=\\""+currentDeposit.amount_collected+"\\" class=\\"w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface\\"><input id=\\"deductionReason\\" class=\\"w-full border border-outline-variant rounded-lg px-3 py-2 mb-2 text-sm bg-surface\\" placeholder=\\"Deduction reason (if any)\\">";}}catch(err){document.getElementById("depositSection").innerHTML="";}}' +
  'function closeReturnModal(){document.getElementById("returnModal").style.display="none";}' +
  'async function confirmReturn(){var body={status:"Available",price:document.getElementById("returnPrice").value};var res=await fetch(qs("motorbikes/"+returnTarget+"/status"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});var data=await res.json();if(data.error||data.ok===false){document.getElementById("returnMsg").textContent=data.error||data.message||"Failed";return;}if(currentDeposit){var refundAmt=currentDeposit.deposit_type==="passport"?0:(document.getElementById("refundAmount")?document.getElementById("refundAmount").value:0);var reason=currentDeposit.deposit_type==="passport"?"":(document.getElementById("deductionReason")?document.getElementById("deductionReason").value:"");await fetch(qs("deposits/"+currentDeposit.id+"/refund"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({amountRefunded:refundAmt,deductionReason:reason})});}closeReturnModal();load();}' +
  'function render(){var f=document.getElementById("statusFilter").value;var bikes=f==="all"?allBikes:allBikes.filter(function(b){return b.status===f;});document.getElementById("bikeCount").textContent=bikes.length+" bike"+(bikes.length!==1?"s":"");document.getElementById("grid").innerHTML=bikes.map(function(b){var p=pill[b.status]||"pill-done";var actionBtn="";if(b.status==="Available"){actionBtn="<button onclick=\\"event.stopPropagation();openRentModal(\'"+b.bikeId+"\')\\" class=\\"text-xs px-2 py-1 rounded-lg bg-primary text-on-primary w-full mt-2\\">Mark Rented</button>";}else if(b.status==="Rented"){actionBtn="<button onclick=\\"event.stopPropagation();openReturnModal(\'"+b.bikeId+"\')\\" class=\\"text-xs px-2 py-1 rounded-lg bg-surface-container-high w-full mt-2\\">Mark Returned</button>";}return "<div onclick=\\"window.location=\'/bike-history/\'+encodeURIComponent(\'"+b.bikeId+"\')+(TOKEN?\'?token=\'+encodeURIComponent(TOKEN):\'\')\\" class=\\"bg-surface-container-lowest border border-outline-variant rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer\\"><div class=\\"flex justify-between items-start mb-2\\"><div><div class=\\"font-label-caps text-lg text-primary\\">"+b.bikeId+"</div><div class=\\"text-sm font-semibold mt-0.5\\">"+b.model+"</div></div><span class=\\"text-xs px-2 py-1 rounded-full font-status-badge "+p+"\\">"+b.status+"</span></div><div class=\\"text-xs text-on-surface-variant space-y-1\\"><div class=\\"flex gap-4\\"><span>Location: "+(b.location||"Chaweng")+"</span>"+(b.color?"<span>Color: "+b.color+"</span>":"")+"</div>"+(b.notes?"<div class=\\"text-on-surface-variant italic opacity-60\\">"+b.notes+"</div>":"")+"</div>"+actionBtn+"</div>";}).join("");}load();setInterval(load,60000);</script>'
, isBossRole(auth)));
});
...
app.get('/calendar', (req, res) => {
  ...
  if (!auth.ok) return res.redirect('/login');
  const token = req.query.token || '';
  res.send(dashboardShell(token, 'Calendar', '', '<main class="flex-1 md:ml-[280px] pb-24 md:pb-8"><header class="hidden md:flex justify-between items-center w-full px-margin-desktop h-16 z-30 bg-surface/80 backdrop-blur-md border-b border-outline-variant sticky top-0"><h2 class="font-headline-md text-headline-md text-on-surface font-semibold">Calendar</h2><div class="flex items-center gap-2"><button onclick="prevMonth()" class="text-xs px-2 py-1 rounded-lg bg-surface-container-high">&lt;</button><span id="monthLabel" class="text-sm font-semibold min-w-[120px] text-center inline-block"></span><button onclick="nextMonth()" class="text-xs px-2 py-1 rounded-lg bg-surface-container-high">&gt;</button></div></header><div class="p-margin-mobile md:p-margin-desktop max-w-4xl mx-auto space-y-3"><div class="flex gap-4 text-xs text-on-surface-variant"><span><span class="inline-block w-2 h-2 rounded-full bg-primary mr-1"></span>Pickup</span><span><span class="inline-block w-2 h-2 rounded-full bg-secondary mr-1"></span>Return</span></div><div id="calGrid" class="grid grid-cols-7 gap-1 text-center"></div><div id="dayDetail"></div></div></main>' +
    '<script>var TOKEN=' + JSON.stringify(token) + ';var allEvents=[];var curYear,curMonth,selectedDate=null;var today=new Date();curYear=today.getFullYear();curMonth=today.getMonth();' +
    'function qs(p){return "/api/toh/"+p+(TOKEN?(p.indexOf("?")>=0?"&":"?")+"token="+encodeURIComponent(TOKEN):"");}' +
    'function parseDMY(s){if(!s)return null;var parts=s.split("/");if(parts.length!==3)return null;return new Date(parseInt(parts[2]),parseInt(parts[1])-1,parseInt(parts[0]));}' +
    'async function loadEvents(){allEvents=[];try{var bRes=await fetch(qs("motorbikes"));var bikes=(await bRes.json()).bikes||[];bikes.forEach(function(b){if(b.status==="Rented"&&b.rentedDate){allEvents.push({date:parseDMY(b.rentedDate),type:"start",plate:b.bikeId,renter:b.renterName||"-"});}if(b.status==="Rented"&&b.expectedReturn){allEvents.push({date:parseDMY(b.expectedReturn),type:"due",plate:b.bikeId,renter:b.renterName||"-"});}});}catch(e){}try{var hRes=await fetch(qs("rental-history"));var hist=(await hRes.json()).history||[];hist.forEach(function(h){if(h.end_date){allEvents.push({date:parseDMY(h.end_date),type:"returned",plate:h.bike_id,renter:h.renter_name||"-"});}});}catch(e){}renderCalendar();}' +
    'function sameDay(a,b){return a&&b&&a.getFullYear()===b.getFullYear()&&a.getMonth()===b.getMonth()&&a.getDate()===b.getDate();}' +
    'var monthNames=["January","February","March","April","May","June","July","August","September","October","November","December"];' +
    'function prevMonth(){curMonth--;if(curMonth<0){curMonth=11;curYear--;}selectedDate=null;renderCalendar();}' +
    'function nextMonth(){curMonth++;if(curMonth>11){curMonth=0;curYear++;}selectedDate=null;renderCalendar();}' +
    'function selectDay(y,m,d){selectedDate=new Date(y,m,d);renderDetail();}' +
    'function renderDetail(){var box=document.getElementById("dayDetail");if(!selectedDate){box.innerHTML="";return;}var dayEvents=allEvents.filter(function(e){return sameDay(e.date,selectedDate);});var label=selectedDate.toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"});if(!dayEvents.length){box.innerHTML="<div class=\\"bg-surface-container-lowest border border-outline-variant rounded-xl p-4 mt-3\\"><div class=\\"font-semibold text-sm mb-2\\">"+label+"</div><div class=\\"text-sm text-on-surface-variant\\">Nothing scheduled</div></div>";return;}var labels={start:"Pickup",due:"Due back",returned:"Returned"};box.innerHTML="<div class=\\"bg-surface-container-lowest border border-outline-variant rounded-xl p-4 mt-3\\"><div class=\\"font-semibold text-sm mb-2\\">"+label+"</div>"+dayEvents.map(function(e){return "<div class=\\"text-sm py-1 border-t border-outline-variant first:border-0\\"><span class=\\"font-semibold\\">"+labels[e.type]+"</span> - "+e.plate+" ("+e.renter+")</div>";}).join("")+"</div>";}' +
    'function renderCalendar(){document.getElementById("monthLabel").textContent=monthNames[curMonth]+" "+curYear;var firstDay=new Date(curYear,curMonth,1);var startWeekday=firstDay.getDay();var daysInMonth=new Date(curYear,curMonth+1,0).getDate();var html="";var dowLabels=["Su","Mo","Tu","We","Th","Fr","Sa"];dowLabels.forEach(function(d){html+="<div class=\\"text-xs font-semibold text-on-surface-variant py-1\\">"+d+"</div>";});for(var i=0;i<startWeekday;i++){html+="<div></div>";}for(var d=1;d<=daysInMonth;d++){var dayDate=new Date(curYear,curMonth,d);var dayEvents=allEvents.filter(function(e){return sameDay(e.date,dayDate);});var dots="";if(dayEvents.some(function(e){return e.type==="start";}))dots+="<span class=\\"inline-block w-1.5 h-1.5 rounded-full bg-primary mx-0.5\\"></span>";if(dayEvents.some(function(e){return e.type==="due"||e.type==="returned";}))dots+="<span class=\\"inline-block w-1.5 h-1.5 rounded-full bg-secondary mx-0.5\\"></span>";var isToday=sameDay(dayDate,today);var isSelected=selectedDate&&sameDay(dayDate,selectedDate);var cls="text-sm py-2 rounded-lg cursor-pointer hover:bg-surface-container-high"+(isToday?" font-bold text-primary":"")+(isSelected?" bg-secondary-container":"");html+="<div class=\\""+cls+"\\" onclick=\\"selectDay("+curYear+","+curMonth+","+d+")\\">"+d+"<div class=\\"h-2\\">"+dots+"</div></div>";}document.getElementById("calGrid").innerHTML=html;renderDetail();}' +
    'loadEvents();</script>', isBossRole(auth)));
});
...
app.get('/staff', (req, res) => {
  ...
  const adminControlsHTML = isBoss
    ...
    : '';
  res.send(dashboardShell(token, 'Staff', '', '<input type="file" id="photoInput" accept="image/*" style="display:none">' + '<main class="flex-1 md:ml-[280px] pb-24 md:pb-8"><header class="flex flex-col md:flex-row justify-between md:items-center w-full px-margin-mobile md:px-margin-desktop py-3 md:h-16 z-30 bg-surface/80 backdrop-blur-md border-b border-outline-variant md:sticky md:top-0 gap-2"><h2 class="font-headline-md text-headline-md text-on-surface font-semibold">Staff</h2><div class="flex items-center gap-3 flex-wrap"><span id="countBadge" class="text-sm text-on-surface-variant"></span>' + adminControlsHTML + '</div></header><div class="p-margin-mobile md:p-margin-desktop max-w-7xl mx-auto space-y-6"><div><h3 class="font-headline-md text-headline-md mb-3">Boss</h3><div id="bossGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div></div><div><h3 class="font-headline-md text-headline-md mb-3">Staff</h3><div id="staffGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div></div></div></main>' +
