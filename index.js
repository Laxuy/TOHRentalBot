...
app.get('/motorbikes', (req, res) => {
  ...
  '<div id="historyModal" class="fixed inset-0 bg-black/40 items-center justify-center z-50" style="display:none"><div class="bg-surface-container-lowest rounded-xl p-6 w-[340px] max-h-[80vh] overflow-y-auto"><div class="flex justify-between items-center mb-4"><h3 id="historyModalTitle" class="font-headline-md text-headline-md">History</h3><button onclick="closeHistoryModal()" class="text-on-surface-variant text-lg leading-none">&times;</button></div><div id="historyModalBody" class="space-y-3 text-sm"></div></div></div>' +
  ...
  'async function openHistoryModal(id,model){document.getElementById("historyModalTitle").textContent=model+" ("+id+")";document.getElementById("historyModalBody").innerHTML="<div class=\\"text-on-surface-variant\\">Loading...</div>";document.getElementById("historyModal").style.display="flex";try{var res=await fetch(qs("motorbikes/"+encodeURIComponent(id)+"/history"));var data=await res.json();var items=data.history||[];if(!items.length){document.getElementById("historyModalBody").innerHTML="<div class=\\"text-on-surface-variant text-center py-6\\">No rental history for this bike yet</div>";return;}document.getElementById("historyModalBody").innerHTML=items.map(function(h){return "<div class=\\"border border-outline-variant rounded-lg p-3\\"><div class=\\"font-semibold\\">"+(h.renter_name||"-")+"</div><div class=\\"text-xs text-on-surface-variant\\">"+(h.start_date||"")+" -> "+(h.end_date||"")+(h.days?" · "+h.days+" days":"")+"</div><div class=\\"text-xs mt-1\\">"+(h.price?h.price+" THB":"-")+(h.logged_by?" · by "+h.logged_by:"")+"</div></div>";}).join("");}catch(err){document.getElementById("historyModalBody").innerHTML="<div class=\\"text-error\\">Failed to load history</div>";}}' +
