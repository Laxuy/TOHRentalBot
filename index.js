...
function dashboardShell(title, bodyAttrs, bodyHTML, isBoss) {
  ...
  return '<!DOCTYPE html><html class="light" lang="en"><head>' +
    ...
    (isBoss ? '<a class="flex items-center gap-4' + active('Overview') + ' px-4 py-3 mx-2" href="/overview"><span class="material-symbols-outlined">dashboard</span><span class="font-label-caps text-label-caps">Overview</span></a>' : '') +
    '<a class="flex items-center gap-4' + active('Motorbikes') + ' px-4 py-3 mx-2" href="/motorbikes"><span class="material-symbols-outlined">two_wheeler</span><span class="font-label-caps text-label-caps">Motorbikes</span></a>' +
    '<a class="flex items-center gap-4' + active('Rentals') + ' px-4 py-3 mx-2" href="/rentals"><span class="material-symbols-outlined">receipt_long</span><span class="font-label-caps text-label-caps">Rentals</span></a>' +
    (isBoss ? '<a class="flex items-center gap-4' + active('AI Task') + ' px-4 py-3 mx-2" href="/ai-tasks"><span class="material-symbols-outlined">smart_toy</span><span class="font-label-caps text-label-caps">AI Tasks</span></a>' : '') +
    (isBoss ? '<a class="flex items-center gap-4' + active('Data Quality') + ' px-4 py-3 mx-2" href="/data-quality"><span class="material-symbols-outlined">verified</span><span class="font-label-caps text-label-caps">Data Quality</span></a>' : '') +
    '<a class="flex items-center gap-4' + active('Rental History') + ' px-4 py-3 mx-2" href="/rental-history"><span class="material-symbols-outlined">history</span><span class="font-label-caps text-label-caps">Rental History</span></a>' +
    '<a class="flex items-center gap-4' + active('Staff') + ' px-4 py-3 mx-2" href="/staff"><span class="material-symbols-outlined">badge</span><span class="font-label-caps text-label-caps">Staff</span></a>' +
...
app.get('/motorbikes', (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.redirect('/login');
  res.send(dashboardShell('Motorbikes Inventory', '', '<main class="flex-1 md:ml-[280px] pb-24 md:pb-8"><header class="hidden md:flex justify-between items-center w-full px-margin-desktop h-16 z-30 bg-surface/80 backdrop-blur-md border-b border-outline-variant sticky top-0"><h2 class="font-headline-md text-headline-md text-on-surface font-semibold">Motorbikes</h2><div class="flex items-center gap-3"><select id="statusFilter" class="text-sm border border-outline-variant rounded-lg px-3 py-1.5 bg-surface" onchange="render()"><option value="all">All Statuses</option><option value="Available">Available</option><option value="Rented">Rented</option><option value="Maintenance">Maintenance</option><option value="Reserved">Reserved</option></select></div></header><div class="p-margin-mobile md:p-margin-desktop max-w-7xl mx-auto space-y-4"><div class="flex justify-between items-center"><div id="bikeCount" class="text-sm text-on-surface-variant"></div></div><div id="grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div></div></main><script>var TOKEN='';var allBikes=[];var pill={Available:"pill-available",Rented:"pill-rented",Maintenance:"pill-maintenance",Reserved:"pill-reserved"};async function load(){try{var res=await fetch("/api/toh/motorbikes");var data=await res.json();if(data.error){document.getElementById("grid").innerHTML="<div class=\\"col-span-full text-error\\">"+data.error+"</div>";return;}allBikes=data.bikes||[];render();}catch(err){document.getElementById("grid").innerHTML="<div class=\\"col-span-full text-error\\">Failed to load</div>";}}function render(){var f=document.getElementById("statusFilter").value;var bikes=f==="all"?allBikes:allBikes.filter(function(b){return b.status===f;});document.getElementById("bikeCount").textContent=bikes.length+" bike"+(bikes.length!==1?"s":"");document.getElementById("grid").innerHTML=bikes.map(function(b){var p=pill[b.status]||"pill-done";return "<div class=\\"bg-surface-container-lowest border border-outline-variant rounded-xl p-4 hover:shadow-sm transition-shadow\\"><div class=\\"flex justify-between items-start mb-2\\"><div><div class=\\"font-label-caps text-lg text-primary\\">"+b.bikeId+"</div><div class=\\"text-sm font-semibold mt-0.5\\">"+b.model+"</div></div><span class=\\"text-xs px-2 py-1 rounded-full font-status-badge "+p+"\\">"+b.status+"</span></div><div class=\\"text-xs text-on-surface-variant space-y-1\\"><div class=\\"flex gap-4\\"><span>Location: "+(b.location||"Chaweng")+"</span>"+(b.color?"<span>Color: "+b.color+"</span>":"")+"</div>"+(b.notes?"<div class=\\"text-on-surface-variant italic opacity-60\\">"+b.notes+"</div>":"")+"</div></div>";}).join("");}load();setInterval(load,60000);</script>', isBossRole(auth)));
});

app.get('/rentals', (req, res) => {
  const auth = checkDashboardAuth(req);
  if (!auth.ok) return res.redirect('/login');
  res.send(dashboardShell('Rentals', '', '<main class="flex-1 md:ml-[280px] pb-24 md:pb-8"><header class="hidden md:flex justify-between items-center w-full px-margin-desktop h-16 z-30 bg-surface/80 backdrop-blur-md border-b border-outline-variant sticky top-0"><h2 class="font-headline-md text-headline-md text-on-surface font-semibold">Rentals</h2><div class="flex items-center gap-3"><select id="statusFilter" class="text-sm border border-outline-variant rounded-lg px-3 py-1.5 bg-surface" onchange="renderRentals()"><option value="all">All</option><option value="active" selected>Active</option><option value="done">Completed</option></select></div></header><div class="p-margin-mobile md:p-margin-desktop max-w-7xl mx-auto space-y-3"><div id="rentalList" class="space-y-3"></div></div></main><script>var TOKEN='';var allRentals=[];async function loadRentals(){try{var bRes=await fetch("/api/toh/motorbikes");var bikes=(await bRes.json()).bikes||[];allRentals=bikes.filter(function(b){return b.status==="Rented";}).map(function(b){return {plate:b.bikeId,model:b.model,renter:b.renterName||"-",status:"active",start:b.rentedDate,end:b.expectedReturn};});try{var hRes=await fetch("/api/toh/rental-history");var history=(await hRes.json()).history||[];history.forEach(function(h){allRentals.push({plate:h.bike_id,model:h.model||"-",renter:h.renter_name||"-",status:"done",start:h.start_date,end:h.end_date,days:h.days,price:h.price});});}catch(e){}renderRentals();}catch(err){document.getElementById("rentalList").innerHTML="<div class=\\"text-error\\">Failed to load</div>";}}function renderRentals(){var f=document.getElementById("statusFilter").value;var items=f==="all"?allRentals:allRentals.filter(function(r){return r.status===f;});document.getElementById("rentalList").innerHTML=items.length?items.map(function(r){return "<div class=\\"bg-surface-container-lowest border border-outline-variant rounded-xl p-4 flex items-center gap-4\\"><div class=\\"font-label-caps text-primary font-bold min-w-[60px]\\">"+r.plate+"</div><div class=\\"flex-1\\"><div class=\\"font-semibold text-sm\\">"+r.renter+"</div><div class=\\"text-xs text-on-surface-variant\\">"+r.model+(r.start?" · "+r.start+" -> "+r.end:"")+(r.days?" · "+r.days+"d":"")+"</div></div><div class=\\"text-right\\"><span class=\\"text-xs px-2 py-1 rounded-full font-status-badge "+(r.status==="active"?"pill-active":"pill-done")+"\\">"+(r.status==="active"?"Active":"Done")+"</span>"+(r.price?"<div class=\\"text-sm font-semibold mt-1 font-label-caps\\">"+r.price+" THB</div>":"")+"</div></div>";}).join(""):"<div class=\\"text-on-surface-variant text-sm text-center py-8\\">No rentals found</div>";}loadRentals();setInterval(loadRentals,60000);</script>', isBossRole(auth)));
});
...
app.get('/staff', (req, res) => {
  ...
  '<script>var TOKEN='';var adminMode=false;var allStaff=[];var editingId=null;' +
  'function qs(p){return "/api/toh/"+p;}' +
